    ...
    inquirer.prompt([{
      ...
      fs.writeFile("orderConfigs/" + answer.name + ".json", JSON.stringify(order, null, 4), function(err) {
        if (err) {
          ...
          console.log("Order Data Saved! To use this same order next time simply");
          var casper = spawn("casperjs", [__dirname + "/spawnOrder.js", answers.password, answers.zip, answer.name]);
